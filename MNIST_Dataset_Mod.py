...
def deepnn(x):
  ...
  with tf.name_scope('fc1'):
    ...
    h_pool2_flat = tf.reshape(h_pool2, [64, 7*7*64])
...
def main(_):
  ...
  with tf.Graph().as_default():
    ...
    with tf.name_scope('loss'):
      cross_entropy = tf.nn.softmax_cross_entropy_with_logits(labels=next_y_test,
                                                              logits=y_conv)
    ...
    with tf.name_scope('accuracy'):
      correct_prediction = tf.equal(tf.argmax(y_conv, 1), tf.argmax(next_y_test, 1))
    ...
    with tf.Session() as sess:
      ...
      sess.run(iterator.initializer, feed_dict={features_placeholder_train: train_images,
          ...
          for step in range(num_steps):
              try:
                  if (step % 50 == 0):
                      ...
                      print(" accuracy at step %d: %f . Time elapsed: %f" % (step, train_accuracy,time_so_far))
                  else:
                    _ = sess.run([train_step], feed_dict={keep_prob: .8})
